...
class ExperimentGrid:
    ...
    def _save_jobs(self):
        ...
        cmd = 'mv "%s" "%s"' % (f.name, self.jobs_pkl)
...
class Parameter:
    ...
    def value(self):
        if self.type == 'int':
            return np.array(self.int_val, dtype='int64')
        elif self.type == 'float':
            return np.array(self.dbl_val)
        else:
            return self.str_val
...
class GridMap:
    ...
    def __init__(self, variables, grid_size):
        ...
        for name, variable in variables.items():
            ...
            type_ = variable['type'].lower().lower()
            if type_ == 'int':
                ...
                raise Exception('unknown parameter type {!r}'.format(type_))
    ...
    def to_unit(self, v):
        ...
        for index, variable in enumerate(self.variables):
            ...
            if type_ == 'int':
                ...
                raise TypeError('unknown parameter type: {!r}'.format(type_))
